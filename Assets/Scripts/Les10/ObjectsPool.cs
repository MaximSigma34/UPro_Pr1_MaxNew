...
public class ObjectsPool : Singleton<ObjectsPool>
{
    ...
    {
        if (!objects.ContainsKey(prefab.GetType().ToString())) objects.Add(prefab.GetType().ToString(), new Stack<GameObject>() );
    ...
    public void ReturnObject<T>(T obj) where T : MonoBehaviour
    {
        ...
        if (!objects.ContainsKey(obj.GetType().ToString())) objects.Add(obj.GetType().ToString(), new Stack<GameObject>());
